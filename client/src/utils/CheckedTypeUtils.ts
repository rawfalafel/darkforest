...
export const locationIdFromHexStr: (location: string) => LocationId = location => {
...
export const locationIdFromDecStr: (location: string) => LocationId = location => {
...
export const locationIdFromBigInt: (location: BigInteger) => LocationId = location => {
  const locationBI = bigInt(location);
  if (locationBI.geq(LOCATION_ID_UB)) throw new Error("not a valid location");
  let ret = locationBI.toString(16);
  while (ret.length < 64) ret = '0' + ret;
...
export const locationIdToDecStr: (locationId: LocationId) => string = locationId => {
